...
public class CherRestauController implements Initializable {
  ...
  private ComboBox<String> min_max;
    ...
    private Pane pane;
    @FXML
    private Button reservation_restaurant;
    @FXML
    ...
    @FXML
    private void reservation_restaurant_action(ActionEvent event) throws IOException {
    	if(StaticVars.currentUser.getRole()==Role.Visitor) {
    		StaticVars.alerte("you must login first");
    	}
    	else {
    		 Parent root = FXMLLoader.load(getClass().getResource("ReservationRestau.fxml"));
             Scene newScene = new Scene(root);
             Stage window = (Stage) ((Node) event.getSource()).getScene().getWindow();
             window.setScene(newScene);
             window.show();
