...
public class PremiumEventController extends AnchorPane implements Initializable,Serializable{
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		tableaffichage.setStyle("-fx-text-inner-color: black;");
		imageview.setStyle("-fx-text-inner-color: black;");
		//searchForum.setStyle("-fx-text-inner-color: white;");
		drawer.blendModeProperty();
    	drawer.setSidePane(drawer);
		...
		try {
			...
			ObservableList<Event> data = FXCollections.observableArrayList(proxy.showAllExperiencesbyUser(StaticVars.currentUser.getId()));
			tableaffichage.setItems(data);
			
			user.setText(StaticVars.currentUser.getFirstName());
			userr.setText(StaticVars.currentUser.getLastName());
	            ...
	            public TableRow<Event> call(TableView<Event> tableView) {
	                ...
	                detailMenuItem.setOnAction(new EventHandler<ActionEvent>() {
	                    ...
	                    public void handle(ActionEvent event) {
	                        ...
	                        navigate("Premium_Consulter_details_event.fxml");
	                    }
		...
		
		{
			...
			showAlerte = true;
		...
		else 
		     {
				...
				e1.setIdUser(StaticVars.currentUser.getId());
    	  ...
    	  try {
            ...
            //  if (!tableaffichage.getSelectionModel().isEmpty()) {
    		  ...
    		  if(tableaffichage.getSelectionModel().getSelectedItem().getNbparticipant() >= 1) {
        			...
        			colonePlaces.setCellValueFactory(new PropertyValueFactory<Event, Integer>("nbplaces"));
        			colonePrice.setCellValueFactory(new PropertyValueFactory<Event, Integer>("price"));
        			ObservableList<Event> data = FXCollections.observableArrayList(companyserviceRemote.showAllExperiencesbyUser(StaticVars.currentUser.getId()));
        			
        			//proxy.showAllExperiencesbyUser(StaticVars.currentUser.getId())
        			tableaffichage.setItems(data);
...
@FXML
public void deleteExpired(ActionEvent event) {
	  ...
	  try {
        		...
        		formatter.parse( datedebut.getValue().toString());
        		
        		 //Expression<javax.sql.Date> date = formatter.parse( datedebut.getValue().toString()).currentDate();
        		companyserviceRemote.DeleteeEventToday(formatter.parse( datedebut.getValue().toString()));
        		//ShowItem();
